...
<head>
    ...
    <style>
        ...
        .btn { background: linear-gradient(135deg, #3b82f6 0%, #10b981 100%); }
        input:focus { outline: none; box-shadow: 0 0 0 3px rgba(59,130,246,0.2); }
    </style>
</head>
    ...
    <div class="w-full max-w-sm">
        ...
        <div class="bg-white rounded-3xl shadow-xl p-7">
            ...
            <form method="POST" action="{{ route('post.register') }}" class="space-y-4">
                ...
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="Ismingiz"
                       class="w-full px-4 py-3.5 rounded-xl border border-gray-200 focus:border-blue-500 text-base">
                ...
                <div class="flex rounded-xl border border-gray-200 overflow-hidden focus-within:border-blue-500">
                    ...
                    <input type="tel" name="phone" value="{{ old('phone') }}" required placeholder="90 123 45 67"
                           class="flex-1 px-4 py-3.5 text-base focus:outline-none" minlength="9" maxlength="9">
                </div>
                ...
                <input type="password" name="password_confirmation" required placeholder="Parolni qayta kiriting"
                       class="w-full px-4 py-3.5 rounded-xl border border-gray-200 focus:border-blue-500 text-base">

                <button type="submit"
                        class="w-full mt-6 py-4 rounded-xl text-white font-bold text-lg btn shadow-md hover:shadow-lg transition">
                    Davom etish →
                </button>
            </form>
